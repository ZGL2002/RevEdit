...
\textbf{Clean fine-tuning does not wash the watermark at default strength.}
Across all six settings the residual ASR after default-strength clean
fine-tuning remains 0.57--0.98 with utility \emph{intact or improved}
(e.g., GPT-2 SST-2: \ftGptSst{} residual at \ftGptSstU{} CACC, above the
0.578 clean level). The strength sweeps, however, reveal a precise margin
rather than an absolute guarantee: at the default lr
$5{\times}10^{-5}$ every epoch count retains 0.85--0.99 residual ASR on
both AGNews and MotherTone, but doubling the learning rate to
$1{\times}10^{-4}$ washes AGNews nearly completely (residual 0.03--0.17)
while \emph{raising} its accuracy to 0.90--0.91, and reduces MotherTone to
0.23--0.34. The watermark therefore survives ordinary fine-tuning, but its
safety margin is about $2\times$ the default learning rate on some tasks;
stronger trigger associations would widen this margin.
...
preserves A intact (ASR 1.000), and using both keys in order returns the
state-dict hash to the clean model (\dwHashClean{}). B's apparent residual
after removal (\dwAfterB{}) equals its flip rate on the hash-verified
\emph{clean} model (0.429), i.e., the intrinsic response to the B trigger
rather than any watermark residue---restore is bitwise, so no residue is
possible. Each buyer thus holds a personal, revocable certificate.
